...
void castFromTime(
  ...
  context.applyToSelectedNoThrow(rows, [&](vector_size_t row) {
    ...
    // Get the offset for the session timezone at session start time. This
    // ensures we get the correct offset accounting for DST at the relevant
    // date. toGMT converts FROM local time TO GMT, so the offset is negative of
    // the result
    auto ts = Timestamp::fromMillis(sessionStartTimeMs);
    ts.toGMT(*sessionTimeZone);
    int64_t offsetMillis = ts.toMillis() - sessionStartTimeMs;
    // The offset is negative because toGMT added the offset to convert to GMT
    int16_t offsetMinutes = static_cast<int16_t>(-offsetMillis / (60 * 1000));
 ...
 public:
  ...
  void castTo(
    ...
    if (input.typeKind() == TypeKind::BIGINT && input.type()->isTime() &&
      ...
      // Get the offset for the session timezone at session start time
      // (or epoch if not set)
      auto ts = Timestamp::fromMillis(sessionStartTimeMs);
      ts.toGMT(*sessionTimeZone);
      int64_t offsetMillis = ts.toMillis() - sessionStartTimeMs;
